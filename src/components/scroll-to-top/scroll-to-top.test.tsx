...
let mockTriggerValue = false
vi.mock('@mui/material', async () => {
  const actual = await vi.importActual('@mui/material') as typeof import('@mui/material')
  return {
        ...actual,
    useScrollTrigger: vi.fn(() => mockTriggerValue),
      }
})

describe('ScrollToTop', () => {
  beforeEach(() => {
    mockTriggerValue = false
     })
  ...
  it('renders visible when scrolled (Zoom in=true)', () => {
    mockTriggerValue = true
    const { container } = render(
        <ThemeProvider theme={theme}>
          <ScrollToTop />
        </ThemeProvider>
        )
        // When scrolled, the Zoom component should show the button
    const box = container.querySelector('[role="presentation"]')
    expect(box).toBeTruthy()
      })
})
