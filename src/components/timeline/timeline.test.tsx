...
describe('Timeline', () => {
  ...
  it('filters items by type when a filter button is clicked', () => {
    ...
    expect(screen.getByText('Work Item')).toBeInTheDocument()
        ...
        // Click the Education filter button — when all filters are active,
          // clicking one sets the filter to only that type
    const educationButton = screen.getByRole('button', { name: 'Education' })
    fireEvent.click(educationButton)

        // After toggling Education off, only Work items should remain visible
        // The Education item should be hidden from the DOM
    expect(screen.getByText('Education Item')).toBeInTheDocument()
        })
