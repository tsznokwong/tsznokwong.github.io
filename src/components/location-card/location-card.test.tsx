...
describe('LocationCard', () => {
  ...
  it('renders color indicator box as CardContent child', () => {
    const { container } = render(
        <ThemeProvider theme={theme}>
          <LocationCard location={mockLocation} />
        </ThemeProvider>
        )
        // The color indicator Box is a direct child of CardContent
    const content = container.querySelector('.MuiCardContent-root')
    expect(content).toBeInTheDocument()
        // CardContent should have exactly 3 children: indicator box, Typography (city), Typography (description)
    const children = Array.from(content!.children)
    expect(children.length).toBe(3)
        // The first child is the color indicator (a Box rendered as div)
    const indicator = children[0] as HTMLDivElement
    expect(indicator.tagName).toBe('DIV')
    expect(indicator.className).toContain('MuiBox-root')
      })
})
